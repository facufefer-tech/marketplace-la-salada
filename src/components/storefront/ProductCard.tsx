...
export function ProductCard({ producto }: { producto: UIProduct }) {
  ...
  return (
    <article className="group overflow-hidden rounded-2xl border border-zinc-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-lg">
      <Link href={`/${slug}/producto/${producto.id}`} className="relative block aspect-[4/5] overflow-hidden bg-zinc-100">
        <Image
      ...
      <div className="space-y-2 p-3">
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            <p className="line-clamp-2 text-sm font-semibold text-zinc-900">{producto.nombre}</p>
            <p className="text-xs text-zinc-500">{producto.tiendas?.nombre ?? "Tienda La Salada"}</p>
          </div>
          <button type="button" onClick={() => setFav((v) => !v)} className="text-lg leading-none text-zinc-400">
            {fav ? "♥" : "♡"}
          </button>
        </div>
        <div>
          {producto.precioOriginal && <p className="text-xs text-zinc-400 line-through">${producto.precioOriginal.toLocaleString("es-AR")}</p>}
          <p className="text-lg font-extrabold text-zinc-900">${Number(producto.precio).toLocaleString("es-AR")}</p>
        </div>
        <button
          type="button"
          onClick={() => add(producto)}
          className="w-full rounded-xl bg-orange-500 px-3 py-2 text-sm font-bold text-white hover:bg-orange-600"
        >
          Agregar al carrito
        </button>
      </div>
