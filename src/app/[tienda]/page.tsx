      ...
      <main className="container-shell py-10">
        <section className="rounded-2xl border border-[#E0E0E0] bg-white p-8 text-center">
          ...
          <div className="mt-5 flex justify-center gap-3">
            ...
            <Link href="/" className="rounded-xl bg-[#F97316] px-4 py-2 font-bold text-white">
              Ir al inicio
            </Link>
          </div>
        </section>
    ...
    <main className="container-shell py-8">
      <div className="relative mb-8 overflow-hidden rounded-2xl border border-[#E0E0E0] bg-white shadow-sm">
        {viewStore.banner_url ? (
          ...
          <div className="h-32 bg-gradient-to-r from-[#F97316] to-[#EA6C0A] sm:h-40" />
        )}
      ...
      <section className="mb-5 rounded-2xl border border-[#E0E0E0] bg-white p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <Link href={`/${params.tienda}`} className={`rounded-full px-3 py-1 text-xs ${fCat === "Todas" ? "bg-[#F97316] text-white" : "bg-[#F5F5F5] text-[#1A1A1A]"}`}>
            Todas
          </Link>
          {categorias.map((c) => (
            <Link key={c} href={`/${params.tienda}?categoria=${encodeURIComponent(c)}`} className={`rounded-full px-3 py-1 text-xs ${fCat === c ? "bg-[#F97316] text-white" : "bg-[#F5F5F5] text-[#1A1A1A]"}`}>
              {c}
            </Link>
          ))}
        </div>
        ...
        {listFiltered.map((p) => {
          ...
          return (
            <Link
              ...
              className="group overflow-hidden rounded-xl border border-[#E0E0E0] bg-white shadow-sm transition hover:-translate-y-0.5 hover:border-[#F97316]/70"
            >
              <div className="relative aspect-[4/5] bg-[#F5F5F5]">
                ...
                {p.destacado && (
                  <span className="absolute left-2 top-2 rounded-full bg-[#F97316] px-2 py-0.5 text-xs font-bold text-white">
                    Destacado
                  </span>
                )}
              </div>
              <div className="p-3">
                <p className="line-clamp-2 text-sm font-semibold text-[#1A1A1A] group-hover:text-[#F97316]">{p.nombre}</p>
                {maybeOriginal && (
